...
router.post('/tasks', requireAgent, (req, res) => {
  ...
  logAudit(job_id || null, 'agent_task_created', `Agent created task: ${task.title}`, req.agent.name);
...
router.post('/message', requireAgent, (req, res) => {
  ...
  const result = db.prepare(
    "INSERT INTO agent_messages (agent_id, direction, message) VALUES (?, 'inbound', ?)"
  ).run(req.agent.id, message.slice(0, 10000));

  logAudit(null, 'agent_message_inbound', `Agent message: ${message.slice(0, 200)}`, req.agent.name);
...
router.post('/:id/send', requireAuth, requireRole('admin', 'pm', 'system_admin'), async (req, res) => {
  const db = getDb();
  ...
  logAudit(null, 'agent_message_outbound', `Admin → ${agent.name}: ${message.slice(0, 200)}`, req.session?.name || 'admin');
