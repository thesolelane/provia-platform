...
const VALID_STAGES = [
  'incoming', 'callback_done', 'appointment_booked', 'site_visit_complete',
  'quote_draft', 'quote_sent', 'follow_up_1', 'follow_up_2', 'signed', 'rejected',
];
...
function resolveContact(db, { name, phone, email, address, city }) {
  ...
  if (!contact && email) {
    contact = db.prepare('SELECT * FROM contacts WHERE email = ? COLLATE NOCASE LIMIT 1').get(email.trim());
  }
  ...
  if (contact) {
    const sets = [];
    const vals = [];
    if (!contact.phone && phone && phone !== 'Unknown number') { sets.push('phone = ?'); vals.push(phone.trim()); }
    if (!contact.email && email) { sets.push('email = ?'); vals.push(email.trim()); }
    if (!contact.address && address) { sets.push('address = ?'); vals.push(address.trim()); }
    if (!contact.city && city) { sets.push('city = ?'); vals.push(city.trim()); }
    if (sets.length) {
  ...
  return findOrCreateContact(db, {
    name, phone, email: email || '', address: address || '', city: city || '', state: 'MA',
  });
}
...
function autoTask(db, lead, nextStage, performer) {
  ...
  const taskDefs = {
    ...
    quote_draft: {
      title:       `📋 Prepare quote: ${name}${addr ? ' — ' + addr : ''}`,
    ...
    follow_up_1: {
      ...
      description: `First follow-up call after quote sent to ${name}.\nPhone: ${phone}`,
      priority:    'normal',
  ...
  try {
    ...
    logAudit(null, 'lead_auto_task', `Lead #${lead.id}: task #${row.lastInsertRowid} auto-created for stage ${nextStage}`, performer);
...
router.get('/', requireAuth, (req, res) => {
  try {
    ...
    const leads    = db.prepare('SELECT * FROM leads WHERE archived = ? ORDER BY updated_at DESC').all(archived);
    res.json({ leads });
  } catch (err) {
...
router.post('/', requireAuth, (req, res) => {
  try {
    const db = getDb();
    const {
      caller_name, caller_phone, source = 'other', notes = '',
    ...
    `).run(caller_name.trim(), caller_phone.trim(), src, notes || '', job_address, job_city, job_email, job_scope, jt);
...
router.patch('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const {
      stage, notes, archive_reason,
      appointment_at, job_address, job_city, job_email, job_scope, job_type,
    } = req.body;
    ...
    if (job_type !== undefined && (VALID_JOB_TYPES.includes(job_type) || job_type === '')) {
      updates.push('job_type = ?'); params.push(job_type);
    }
    if (appointment_at !== undefined) {
      updates.push('appointment_at = ?'); params.push(appointment_at || null);
    }
    ...
    if (stage !== undefined) {
      ...
      if (stage === 'rejected') {
        ...
        if (archive_reason) {
          ...
          updates.push('archive_reason = ?'); params.push(reason);
        }
      }
      ...
      if (stage === 'quote_sent' && lead.stage !== 'quote_sent' && !lead.contact_id) {
        try {
          ...
          updates.push('contact_id = ?');          params.push(ref.id);
          updates.push('pb_customer_number = ?');   params.push(ref.pb_customer_number || '');
          merged.contact_id = ref.id;
          logAudit(null, 'lead_contact_created',
      ...
      if (stage === 'signed' && lead.stage !== 'signed') {
        try {
          ...
          logAudit(null, 'lead_contract_task_created',
            `Lead #${leadId}: contract task created for ${lead.caller_name}`, performer);
        } catch (e) {
          console.error('[Leads] contract task error:', e.message);
        }
      }

      logAudit(null, 'lead_stage_changed',
        `Lead #${leadId}: ${lead.stage} → ${stage} by ${performer}`, performer);
    }
...
router.delete('/:id', requireAuth, (req, res) => {
  try {
    ...
    logAudit(null, 'lead_deleted',
      `Lead #${req.params.id} deleted — ${lead.caller_name}`, req.session?.name || 'admin');
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
