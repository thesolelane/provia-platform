...
router.post('/wizard', requireAuth, async (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE jobs SET pb_number = ?, external_ref = ?, quote_number = ? WHERE id = ?')
  ...
  (async () => {
    try {
      ...
      const { context: priorCtx } = findPriorQuoteContext(db, {
        rawText: sanitizedScope, contactId: contactRef?.id, projectAddress
      });
        ...
        messages: [
          {
            ...
            content: [
              ...
              { type: 'text', text: 'This is a construction estimate. Extract the TECHNICAL SCOPE ONLY: line items, quantities, dollar amounts, material specs, trade names. Do NOT include customer names, emails, or phone numbers. Format as plain text.' }
            ]
          }
        ]
      });
  ...
  if (plansTempId) {
    try {
      ...
      if (fs.existsSync(tempDir)) {
        ...
        let existing = [];
        try {
          existing = JSON.parse(db.prepare('SELECT attachments FROM jobs WHERE id = ?').get(jobId)?.attachments || '[]');
        } catch { /* ignore */ }
        db.prepare('UPDATE jobs SET attachments = ? WHERE id = ?')
          .run(JSON.stringify([...existing, ...movedPaths]), jobId);
