...
router.post('/', requireAuth, (req, res) => {
  try {
    ...
    if (file.mimetype && !ALLOWED_MIMES.includes(file.mimetype))        return res.status(400).json({ error: 'Invalid image type' });
    ...
    file.mv(filePath, (err) => {
      if (err) { console.error('Field photo move error:', err); return res.status(500).json({ error: 'Failed to save file' }); }

      const db     = getDb();
      const result = db.prepare(`
        INSERT INTO field_photos (filename, original_name, taken_at, lat, lon, location_label, accuracy, uploaded_by, job_id, lead_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(filename, file.name, taken_at, lat, lon, location_label, accuracy, uploaded_by, job_id, lead_id);

      res.json({ id: result.lastInsertRowid, filename, original_name: file.name, taken_at, lat, lon, location_label, accuracy, job_id, lead_id, uploaded_by });
    });
  } catch (err) {
...
router.get('/', requireAuth, (req, res) => {
  try {
    ...
    if (req.query.lead_id) {
      photos = db.prepare('SELECT * FROM field_photos WHERE lead_id = ? ORDER BY taken_at DESC').all(parseInt(req.query.lead_id, 10));
    } else if (req.query.job_id) {
      photos = db.prepare('SELECT * FROM field_photos WHERE job_id = ? ORDER BY taken_at DESC').all(req.query.job_id);
    } else {
...
router.patch('/:id/assign', requireAuth, (req, res) => {
  try {
    ...
    if (job_id) {
      ...
      db.prepare('UPDATE field_photos SET job_id = ?, lead_id = NULL WHERE id = ?').run(job_id, req.params.id);
      ...
      db.prepare(`INSERT OR IGNORE INTO job_photos (job_id, filename, original_name, caption) VALUES (?, ?, ?, '')`)
    ...
    if (lead_id) {
      ...
      db.prepare('UPDATE field_photos SET lead_id = ?, job_id = NULL WHERE id = ?').run(parseInt(lead_id, 10), req.params.id);
      return res.json({ ok: true, lead_id: parseInt(lead_id, 10), job_id: null });
    }

    // Clear assignment (null both)
    db.prepare('UPDATE field_photos SET job_id = NULL, lead_id = NULL WHERE id = ?').run(req.params.id);
