 ...
 *
 * Drop-in replacement for server/services/pdfService.js
 *
 * Exports:
 *   buildContractHTML(data)  — full 16-article contract + Exhibit A + Addendum 1
 *   buildProposalHTML(data)  — proposal & scope of work (structure preserved from
 *                              your existing builder — wire your fields in here)
 *
 * Usage (unchanged from your existing service):
 *   const { buildContractHTML, buildProposalHTML } = require('./pdfService');
 *   const html = buildContractHTML(data);
 *   // pass html to Puppeteer as before
 *
 * Data shape expected by buildContractHTML — see DATA SCHEMA section below.
 * All fields are strings unless noted. Missing fields render as empty string.
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
//  DATA SCHEMA  (what your scope-of-work system passes in)
// ═══════════════════════════════════════════════════════════════════════════════
//
// data = {
//   contract: {
//     contract_number:      string,   e.g. "PB-2026-001"
//     invoice_number:       string,   e.g. "INV-2026-001"
//     quote_number:         string,   e.g. "Q-2026-001"
//     proposal_date:        string,   e.g. "March 12, 2026"
//     contract_date:        string,   e.g. "March 12, 2026"
//     total_contract_price: string,   e.g. "$185,000"
//     deposit_amount:       string,   e.g. "$61,666"
//     deposit_pct:          string,   e.g. "33%"
//   },
//   owner: {
//     full_name:            string,
...
//     address:              string,
//     city:                 string,
//     jurisdiction:         string,   e.g. "City of Worcester"
...
//     engineer_fee:         string,
//     has_architect:        bool,
//     architect_fee:        string,
//     sub_deposits:         string,
//     special_order_deposits: string,
//     trades: {
//       electrical:         bool,
//       plumbing:           bool,
//       hvac:               bool,
...
//       description:        string,
//     },
//     milestoneShares:      object,  // { 'NC-2': '20%', ... }
//     milestoneAmounts:     object,  // { 'NC-2': '$37,000', ... }
//     invoiceNumbers:       object,  // { 'NC-2': 'INV-2026-002', ... }
//     final_milestone_share:  string,
//     final_milestone_amount: string,
//     final_invoice_number:   string,
//   },
//   trades: [                        // array of trade/phase rows for Article I table
//     { phase: string, description: string, value: string }
//   ],
//   allowances: {                    // true = include in Exhibit A
//     flooring_lvp:         bool,
//     flooring_tile:        bool,
//     flooring_carpet:      bool,
//     kitchen_cabinets:     bool,
//     kitchen_counter:      bool,
//     kitchen_faucet:       bool,
//     kitchen_sink:         bool,
//     kitchen_disposal:     bool,
//     bath_vanity_full:     bool,
//     bath_vanity_half:     bool,
//     bath_vanity_top:      bool,
//     bath_faucet:          bool,
//     bath_toilet:          bool,
//     bath_tub:             bool,
//     bath_shower_valve:    bool,
//     bath_shower_door:     bool,
//     bath_accessories:     bool,
//     bath_exhaust_fan:     bool,
//     doors_interior:       bool,
//     doors_passage:        bool,
//     doors_privacy:        bool,
//     doors_bifold:         bool,
//     doors_base_molding:   bool,
//     doors_casing:         bool,
//     doors_window_stool:   bool,
//   }
// }
...
const f = (val, fallback = '') => (val !== undefined && val !== null && val !== '') ? val : fallback;
...
const css = () => `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .clause .cb { flex: 1; }
  .clause .cb b { color: #1a1a2e; }

  /* ── Tables (payment, allowance, library) ── */
  table.data-table {
    width: 100%;
    border-collapse: collapse;
    font-size: 8.5pt;
  ...
  table.data-table .section-hdr td {
    font-weight: 700;
    color: #fff;
    font-size: 8pt;
    padding: 6px 9px;
  ...
  /* ── Field placeholder (shown in blank/template mode) ── */
  .fld {
    color: #8090A8;
    font-style: italic;
  }
  ...
  ul.clause-list li b { color: #1F3864; }
...
// ═══════════════════════════════════════════════════════════════════════════════
//  PRE-CONSTRUCTION ADVANCE TABLE
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildAllowanceTables(data) {
  ...
  const section = (title, rows) => {
    const filtered = rows.filter(r => al[r.key] !== false); // include unless explicitly false
    ...
    return `
      ...
      <table class="data-table">
        ...
        <tbody>${filtered.map((r,i) => `<tr><td>${r.item}</td><td>${r.loc||''}</td><td>${r.allowance}</td><td style="font-size:8pt;color:#555">${r.spec}</td></tr>`).join('')}</tbody>
      </table>`;
  };
...
<div id="page-header">
  ...
  <div class="doc-type">
    Home Improvement Construction Contract<br>
    M.G.L. c. 142A  |  780 CMR Compliant
  </div>
</div>
...
${clauseHTML('2.2', `The Contract Price includes contractor-grade material allowances as set forth in <strong>Exhibit A — Contractor-Grade Allowance Schedule</strong>, attached hereto and incorporated herein.`)}
...
${clauseHTML('6.5', `Owner shall obtain, at Owner's sole expense, any required easements, rights-of-way, or property line clearances necessary for completion of the Work.`)}
${clauseHTML('6.6', `Owner shall be available for scheduled walkthroughs, milestone inspections, and decision points with reasonable notice. Owner's failure to be available shall not delay payment obligations.`)}
${clauseHTML('6.7', `<strong>Owner Acknowledgment — Building Code Compliance.</strong> Owner acknowledges that Contractor is legally required to perform all Work in compliance with 780 CMR and all applicable laws. Owner agrees not to direct, request, or instruct Contractor to perform non-code-compliant work. Any such instruction is void and unenforceable. Owner may not withhold payment or make any claim against Contractor based on Contractor's refusal to perform non-code-compliant work.`)}

<!-- ══ ARTICLE VII ══ -->
<hr class="rule">
<div class="article-heading">Article VII — Permits &amp; Inspections</div>

${clauseHTML('7.1', `Contractor shall apply for and obtain all building permits required by the <strong>${field(p.jurisdiction)}</strong> for the Work described herein pursuant to 780 CMR. Permit fees are included in the Contract Price and itemized in the Pre-Construction Advance schedule in Article 3.3, unless otherwise noted in the Proposal.`)}
${clauseHTML('7.2', `Contractor shall schedule and manage all required inspections including, as applicable to the scope of work: foundation, framing, rough electrical, rough plumbing, insulation, final electrical, final plumbing, and final building inspections. All inspection coordination is included in the Contract Price.`)}
${clauseHTML('7.3', `Contractor shall obtain a Certificate of Occupancy upon Substantial Completion (where required), provided Owner has fulfilled all obligations under this Agreement including timely payment.`)}
${clauseHTML('7.4', `<strong>No Work Without Required Permits.</strong> Contractor shall not commence any work for which a permit is required under 780 CMR or applicable municipal code until the applicable permit has been issued. Any Owner instruction to proceed without a required permit is void and unenforceable.`)}
...
<div class="notary-block">
  ...
  Commonwealth of Massachusetts, County of _________________ ss.
...
//  Wire your existing proposal fields into this structure.
//  The HTML skeleton matches the style of the contract above so both
//  documents share the same visual language.
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildProposalHTML(data) {
  ...
  // ── Replace the body below with your existing proposal content.
  // ── The css(), field(), clauseHTML(), sigBlock() helpers are all available.

  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Project Proposal — ${f(c.quote_number)}</title>
  <style>${css()}</style>
</head>
<body>
...
<div id="page-header">
  ...
  <div class="doc-type">Project Proposal &amp; Scope of Work</div>
</div>
<div id="page-footer">
  <span>jackson.deaquino@preferredbuildersusa.com  |  Quote No.: ${field(c.quote_number)}</span>
  <span>Page <span class="page-num"></span></span>
</div>

<div class="doc-title">
  <h1>Project Proposal<br>&amp; Scope of Work</h1>
  <div class="subtitle">Quote No. ${field(c.quote_number)}  |  ${field(c.proposal_date)}</div>
</div>

<hr class="rule">

<!-- ── Wire your existing proposal sections here ── -->
<!-- Example: owner info, property, line items, scope descriptions, pricing, terms -->

<p style="font-size:9pt;color:#888;font-style:italic;text-align:center;margin:40px 0">
  [ Proposal content — wire your existing buildProposalHTML fields here ]
</p>
...
module.exports = { buildContractHTML, buildProposalHTML };
