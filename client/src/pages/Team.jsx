...
const inp = { width: '100%', padding: '9px 10px', border: '1px solid #ddd', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' };
...
export default function Team({ token, userRole }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'pm', title: 'Team Member', phone: '', language: 'en' });
  const [senderForm, setSenderForm] = useState({ identifier: '', type: 'email', name: '', role: 'pm', language: 'en' });
  const [pwdForm, setPwdForm] = useState({ currentPassword: '', newPassword: '', confirm: '' });
  const [pwdErr, setPwdErr] = useState('');

  const headers = { 'x-auth-token': token, 'Content-Type': 'application/json' };

  const loadUsers = () => fetch('/api/users', { headers: { 'x-auth-token': token } })
    .then(r => r.json()).then(d => setUsers(Array.isArray(d) ? d : []));

  const loadSenders = () => fetch('/api/whitelist', { headers: { 'x-auth-token': token } })
    .then(r => r.json()).then(d => setSenders(Array.isArray(d) ? d : []));

  useEffect(() => { loadUsers(); loadSenders(); }, []);

  const addUser = async () => {
    if (!form.name || !form.email || !form.password) return;
    const res = await fetch('/api/users', { method: 'POST', headers, body: JSON.stringify(form) });
    if (res.ok) { setShowAdd(false); setForm({ name: '', email: '', password: '', role: 'pm', title: 'Team Member', phone: '', language: 'en' }); loadUsers(); }
    else { const d = await res.json(); alert(d.error || 'Error'); }
  };

  const saveEdit = async () => {
    const res = await fetch(`/api/users/${editUser.id}`, { method: 'PUT', headers, body: JSON.stringify(editUser) });
    if (res.ok) { setEditUser(null); loadUsers(); }
  };

  const toggleUser = async (u) => {
    await fetch(`/api/users/${u.id}`, { method: 'PUT', headers, body: JSON.stringify({ active: !u.active }) });
    loadUsers();
  };
  ...
  const savePassword = async () => {
    ...
    const res = await fetch(`/api/users/${pwdUser.id}/password`, { method: 'PUT', headers, body: JSON.stringify(pwdForm) });
    if (res.ok) { setPwdUser(null); setPwdForm({ currentPassword: '', newPassword: '', confirm: '' }); }
    else { const d = await res.json(); setPwdErr(d.error || 'Error'); }
  };
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 'bold', color: BLUE, margin: 0 }}>👷 Team & Access</h1>
          <p style={{ color: '#888', fontSize: 13, marginTop: 4 }}>Manage team members, logins, and bot contacts</p>
        </div>
        {tab === 'team' && isSysAdmin && (
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 24, borderBottom: '2px solid #e0e0e0' }}>
        {[['team', '🧑‍💼 Team Members'], ['bot', '🤖 Bot Contacts']].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)} style={{
            padding: '10px 24px', border: 'none', borderBottom: tab === key ? `2px solid ${BLUE}` : '2px solid transparent',
            background: 'none', color: tab === key ? BLUE : '#888', fontWeight: tab === key ? 'bold' : 'normal',
        ...
        <>
          ...
          <div style={{ background: 'white', borderRadius: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 620 }}>
              ...
              <tbody>
                {senders.length === 0 && (
                  <tr><td colSpan={7} style={{ padding: 32, textAlign: 'center', color: '#888' }}>No bot contacts yet.</td></tr>
                )}
                {senders.map((s, i) => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 32, width: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[['Name', 'name', 'Full name'], ['Email', 'email', 'Login email']].map(([label, key, ph]) => (
                <div key={key}>
                  <label style={lbl}>{label}</label>
                  <input value={form[key]} placeholder={ph} onChange={e => setForm({ ...form, [key]: e.target.value })} style={inp} />
                </div>
              ))}
              <div>
              ...
              <div>
                ...
                <input value={form.title} placeholder="e.g. Project Manager" onChange={e => setForm({ ...form, title: e.target.value })} style={inp} />
              </div>
              <div>
                <label style={lbl}>WhatsApp Phone</label>
                <input value={form.phone} placeholder="+11234567890" onChange={e => setForm({ ...form, phone: e.target.value })} style={inp} />
              </div>
              <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 8, padding: '8px 12px', background: '#f8f8f8', borderRadius: 6 }}>
              <strong>Admin</strong> — jobs, tasks, contacts, settings &nbsp;|&nbsp; <strong>System Admin</strong> — + secrets, user management
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={addUser} disabled={!form.name || !form.email || !form.password}
      ...
      {editUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 32, width: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ color: BLUE, marginBottom: 20, marginTop: 0 }}>Edit Profile — {editUser.name}</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <label style={lbl}>Name</label>
                <input value={editUser.name} onChange={e => setEditUser({ ...editUser, name: e.target.value })} style={inp} />
              </div>
              <div>
                <label style={lbl}>Job Title</label>
                <input value={editUser.title} onChange={e => setEditUser({ ...editUser, title: e.target.value })} style={inp} />
              </div>
              <div>
                <label style={lbl}>WhatsApp Phone</label>
                <input value={editUser.phone} placeholder="+11234567890" onChange={e => setEditUser({ ...editUser, phone: e.target.value })} style={inp} />
              </div>
              <div>
                <label style={lbl}>Language</label>
                <select value={editUser.language} onChange={e => setEditUser({ ...editUser, language: e.target.value })} style={inp}>
                  <option value="en">🇺🇸 English</option>
                  <option value="pt-BR">🇧🇷 Português</option>
                </select>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => setEditUser(null)} style={{ flex: 1, padding: 10, border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', background: 'white' }}>Cancel</button>
      ...
      {pwdUser && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 32, width: 400 }}>
            <h2 style={{ color: BLUE, marginBottom: 20, marginTop: 0 }}>Change Password — {pwdUser.name}</h2>
            {!isSysAdmin && (
              <div style={{ marginBottom: 12 }}>
                <label style={lbl}>Current Password</label>
                <input type="password" value={pwdForm.currentPassword} onChange={e => setPwdForm({ ...pwdForm, currentPassword: e.target.value })} style={inp} />
              </div>
            )}
            <div style={{ marginBottom: 12 }}>
              <label style={lbl}>New Password</label>
              <input type="password" value={pwdForm.newPassword} onChange={e => setPwdForm({ ...pwdForm, newPassword: e.target.value })} style={inp} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={lbl}>Confirm New Password</label>
              <input type="password" value={pwdForm.confirm} onChange={e => setPwdForm({ ...pwdForm, confirm: e.target.value })} style={inp} />
            </div>
            {pwdErr && <div style={{ color: RED, fontSize: 12, marginBottom: 10 }}>{pwdErr}</div>}
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setPwdUser(null)} style={{ flex: 1, padding: 10, border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', background: 'white' }}>Cancel</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 32, width: 440 }}>
            ...
            {[
              ...
              { label: 'Email or WhatsApp (with country code)', key: 'identifier', placeholder: 'email@example.com or +11234567890' },
            ].map(f => (
              <div key={f.key} style={{ marginBottom: 12 }}>
                <label style={lbl}>{f.label}</label>
                <input value={senderForm[f.key]} placeholder={f.placeholder} onChange={e => setSenderForm({ ...senderForm, [f.key]: e.target.value })} style={inp} />
              </div>
            ))}
            {[
              { label: 'Type', key: 'type', options: [['email', '📧 Email'], ['whatsapp', '📱 WhatsApp']] },
              { label: 'Role', key: 'role', options: [['system_admin', 'System Admin'], ['admin', 'Admin'], ['pm', 'Project Manager'], ['staff', 'Staff']] },
              { label: 'Language', key: 'language', options: [['en', '🇺🇸 English'], ['pt-BR', '🇧🇷 Português']] },
            ].map(f => (
              <div key={f.key} style={{ marginBottom: 12 }}>
                <label style={lbl}>{f.label}</label>
                <select value={senderForm[f.key]} onChange={e => setSenderForm({ ...senderForm, [f.key]: e.target.value })} style={inp}>
                  {f.options.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                </select>
              </div>
            ))}
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button onClick={() => setShowAddSender(false)} style={{ flex: 1, padding: 10, border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', background: 'white' }}>Cancel</button>
              <button onClick={addSender} disabled={!senderForm.identifier}
                style={{ flex: 2, padding: 10, background: BLUE, color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}>
                Add Contact
              </button>
            </div>
