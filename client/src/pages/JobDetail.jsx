...
export default function JobDetail({ token, userName }) {
  ...
  // ── RFQ state ────────────────────────────────────────────────────────────────
  const [rfqModal, setRfqModal] = useState(null); // null | { trade, baseCost, description }
  const [rfqList, setRfqList] = useState([]);
  const [rfqForm, setRfqForm] = useState({ scopeText: '', vendorId: '', vendorName: '', vendorEmail: '', dueDate: '', generating: false, saving: false, sending: false });
  const [vendors, setVendors] = useState([]);

  const openRfqModal = (li) => {
    setRfqForm({ scopeText: li.description || '', vendorId: '', vendorName: '', vendorEmail: '', dueDate: '', generating: false, saving: false, sending: false });
  ...
  const generateRfqScope = async () => {
    ...
    try {
      ...
      else { setRfqForm((f) => ({ ...f, generating: false })); showToast(data.error || 'Generation failed', 'error'); }
    } catch { setRfqForm((f) => ({ ...f, generating: false })); showToast('Network error', 'error'); }
  };
  ...
  const saveRfq = async () => {
    ...
    try {
      ...
      } else { setRfqForm((f) => ({ ...f, saving: false })); showToast(data.error || 'Save failed', 'error'); }
    } catch { setRfqForm((f) => ({ ...f, saving: false })); showToast('Network error', 'error'); }
  };
  ...
  const sendRfq = async (rfqId) => {
    ...
    try {
      ...
      } else { setRfqForm((f) => ({ ...f, sending: false })); showToast(data.error || 'Send failed', 'error'); }
    } catch { setRfqForm((f) => ({ ...f, sending: false })); showToast('Network error', 'error'); }
  };
  ...
  // Load POs when tab becomes active
  useEffect(() => {
  ...
  return (
      ...
      >
                ...
                [...auditLog]
                  ...
                  .map((a) => (
                    ...
                    >
                      ...
                      <span
                        style={{ fontWeight: 600, color: ORANGE, flexShrink: 0, minWidth: 160 }}
                      >
        ...
        {activeTab === 'payments' && (
          <div>
            ...
            <ActivityLog
              jobId={id}
              customerNumber={job?.contact?.pb_customer_number || null}
              token={token}
              collapsed={true}
            />
          </div>
        ...
        {activeTab === 'assessment' &&
          (() => {
            ...
            const normalizeType = (t) => {
              ...
              if (s.includes('new_construct') || s.includes('custom_home'))
            ...
            const expectedTrades = expectedTradesKey
            ...
            return (
              <div>
                ...
                <div style={{ marginBottom: 28 }}>
                  <h3 style={{ color: BLUE, marginBottom: 16, marginTop: 0 }}>
                  ...
                  {marginLoading ? (
                    ...
                    >
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead>
                          <tr style={{ background: BLUE, color: 'white' }}>
                            <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600 }}>
                              Layer
                            </th>
                            <th
                              style={{ padding: '9px 14px', textAlign: 'right', fontWeight: 600 }}
                            >
                              Target %
                            </th>
                            <th
                            ...
                            <th
                              style={{ padding: '9px 14px', textAlign: 'center', fontWeight: 600 }}
                            >
                              Pass / Fail
                            </th>
                          </tr>
                        ...
                        <tbody>
                          ...
                          <tr style={{ background: '#f8f9fa', borderBottom: '1px solid #e9ecef' }}>
                            ...
                            <td style={{ padding: '10px 14px', textAlign: 'right', color: '#aaa' }}>
                              —
                            </td>
                            <td style={{ padding: '10px 14px', textAlign: 'right', color: '#aaa' }}>
                              —
                            ...
                            <td
                              style={{ padding: '10px 14px', textAlign: 'center', color: '#aaa' }}
                            >
                              —
                            </td>
                          </tr>
                          ...
                          {marginData.layers.map((layer, i) => (
                            ...
                            >
                              ...
                              <td
                                style={{ padding: '10px 14px', textAlign: 'right', color: '#777' }}
                              >
                              ...
                              <td
                                style={{ padding: '10px 14px', textAlign: 'right', color: '#555' }}
                              >
                          ...
                          >
                            ...
                            <td style={{ padding: '12px 14px', textAlign: 'right', color: '#aaa' }}>
                              —
                            </td>
                            <td style={{ padding: '12px 14px', textAlign: 'right', color: '#aaa' }}>
                              —
                        ...
                        <div style={{ fontSize: 13, color: '#555' }}>
                          ...
                          {!marginData.hasStoredRates && (
                            <div style={{ fontSize: 10, color: '#aaa', marginTop: 2 }}>
                              * Actual % assumed from current settings (proposal predates rate
                              tracking)
                            </div>
                          )}
                        </div>
                ...
                {!pd ? (
                  <div style={{ color: '#888', textAlign: 'center', padding: 40 }}>
                    ...
                    <div style={{ fontWeight: 600 }}>
                      Generate an estimate first to see the assessment.
                    </div>
                    <div style={{ fontSize: 12, color: '#aaa', marginTop: 6 }}>
                  ...
                  <>
                      ...
                      >
                        ...
                        {!bandStatus && computedSqftWarning && (
                          ...
                          >
                            ...
                            {sqftTargetLow}–{sqftTargetHigh}/sqft)
                          </div>
                        )}
                    ...
                    {pipelineCtx && (
                      ...
                      >
                        <div
                          style={{ fontWeight: 600, fontSize: 13, color: BLUE, marginBottom: 12 }}
                        >
                          Pipeline Context
                        </div>
                        <div
                          ...
                          <div style={{ textAlign: 'center' }}>
                            <div
                              style={{
                                ...
                                fontWeight: 700,
                                color:
                                  pipelineCtx.daysAtCurrentStage >
                                  (pipelineCtx.avgDaysToClose || 30)
                                    ? '#991b1b'
                        ...
                        {pipelineCtx.daysAtCurrentStage > 14 && (
                          ...
                          >
                            ...
                            <div style={{ color: '#7f1d1d', marginBottom: 8 }}>
                              This job has been at <em>{job.status?.replace(/_/g, ' ')}</em> for
                              over 2 weeks. Consider reaching out to the customer to keep momentum.
                            </div>
                            {followUpTask === null && (
                              <button
                                onClick={async () => {
                    ...
                    {missingTrades.length > 0 && (
                      ...
                      >
                        ...
                        <div style={{ fontSize: 11, color: '#b45309', marginTop: 8 }}>
                          These trades are typically scoped in a{' '}
                          {band ? band.label : 'this type of project'} but appear missing from the
                          estimate. Confirm with the sub or verify the scope intentionally excludes
                          them.
                          {isADU &&
                            aduOnSeptic &&
                    ...
                    {missingTrades.length === 0 && expectedTrades.length > 0 && (
                      ...
                      >
                        ...
                        {band ? band.label : projType || 'this'} project
                      </div>
                    )}
                      ...
                      <tbody>
                        {lineItems.map((li, i) => {
                          ...
                          return (
                            <tr
                              key={i}
                              style={{ background: rowBg, borderBottom: '1px solid #eee' }}
                            >
                              <td style={{ padding: '8px 10px', color: '#333' }}>{li.trade}</td>
                              <td
                                style={{ padding: '8px 10px', textAlign: 'right', color: '#777' }}
                              >
                                ${bc.toLocaleString()}
                              </td>
                              <td
                    ...
                    {isLocked && (
                      ...
                      >
                        ...
                        preserved as a permanent historical record for this job.
                      </div>
                    )}
                  </>
                )}
    ...
    {rfqModal && (
      <div
        ...
        onClick={(e) => { if (e.target === e.currentTarget) setRfqModal(null); }}
      >
        <div style={{ background: 'white', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.18)' }}>
          {/* Header */}
          <div style={{ background: BLUE, padding: '16px 22px', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ color: '#F5A623', fontWeight: 700, fontSize: 15 }}>Request for Quote</div>
              <div style={{ color: '#ccc', fontSize: 12, marginTop: 2 }}>{rfqModal.trade} — {job?.project_address || 'Project'}</div>
            </div>
            <button onClick={() => setRfqModal(null)} style={{ background: 'none', border: 'none', color: 'white', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>✕</button>
          </div>

          <div style={{ padding: '20px 22px' }}>
            {/* Scope text */}
            <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 6 }}>Scope of Work</label>
            <textarea
              ...
              style={{ width: '100%', border: '1px solid #dde3ed', borderRadius: 8, padding: '10px 12px', fontSize: 13, resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.5, boxSizing: 'border-box' }}
            />
            <button
              onClick={generateRfqScope}
              disabled={rfqForm.generating}
              style={{ marginTop: 6, background: rfqForm.generating ? '#ccc' : '#EEF3FB', color: BLUE, border: `1px solid ${BLUE}`, borderRadius: 6, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: rfqForm.generating ? 'not-allowed' : 'pointer' }}
            >
              {rfqForm.generating ? '⏳ Generating…' : '✨ AI-Generate Scope'}
            </button>

            {/* Vendor picker */}
            <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginTop: 16, marginBottom: 6 }}>Sub / Vendor</label>
            <select
              value={rfqForm.vendorId}
              onChange={(e) => {
                const v = vendors.find((x) => String(x.id) === e.target.value);
                setRfqForm((f) => ({ ...f, vendorId: e.target.value, vendorName: v?.company_name || '', vendorEmail: v?.email || '' }));
              }}
              style={{ width: '100%', border: '1px solid #dde3ed', borderRadius: 8, padding: '8px 12px', fontSize: 13, background: 'white', boxSizing: 'border-box' }}
            >
              <option value="">— Select vendor (optional) —</option>
              {vendors.map((v) => (
                <option key={v.id} value={v.id}>{v.company_name}{v.trade ? ` — ${v.trade}` : ''}</option>
              ))}
            </select>

            {/* Vendor email override */}
            <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginTop: 12, marginBottom: 6 }}>Vendor Email</label>
            <input
              type="email"
              value={rfqForm.vendorEmail}
              onChange={(e) => setRfqForm((f) => ({ ...f, vendorEmail: e.target.value }))}
              placeholder="vendor@example.com"
              style={{ width: '100%', border: '1px solid #dde3ed', borderRadius: 8, padding: '8px 12px', fontSize: 13, boxSizing: 'border-box' }}
            />

            {/* Due date */}
            <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginTop: 12, marginBottom: 6 }}>Quote Due By</label>
            <input
              type="date"
            ...
            {/* Internal cost note */}
            {rfqModal.baseCost > 0 && (
              <div style={{ marginTop: 12, padding: '8px 12px', background: '#fffbe6', border: '1px solid #f5c842', borderRadius: 6, fontSize: 12, color: '#7a6000' }}>
                Internal target: <strong>${Number(rfqModal.baseCost).toLocaleString()}</strong> sub cost — not shown to vendor
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20, flexWrap: 'wrap' }}>
              <button
                ...
                style={{ background: '#f0fdf4', color: '#166534', border: '1px solid #bbf7d0', borderRadius: 7, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: rfqForm.saving ? 'not-allowed' : 'pointer' }}
              >
                {rfqForm.saving ? 'Saving…' : '💾 Save Draft'}
              </button>
            </div>

            {/* Existing RFQs for this job */}
            {rfqList.filter((r) => r.trade === rfqModal.trade).length > 0 && (
              <div style={{ marginTop: 22, borderTop: '1px solid #eef1f6', paddingTop: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#888', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.3px' }}>Saved RFQs — {rfqModal.trade}</div>
                {rfqList.filter((r) => r.trade === rfqModal.trade).map((r) => (
                  <div key={r.id} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 14px', marginBottom: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, color: '#888' }}>
                          {r.created_date}
                          {r.vendor_name && <span style={{ marginLeft: 8, color: '#555' }}>→ {r.vendor_name}</span>}
                          {r.status === 'sent' && <span style={{ marginLeft: 8, color: '#166534', fontWeight: 600 }}>✓ Sent {r.sent_date}</span>}
                          {r.status === 'draft' && <span style={{ marginLeft: 8, color: '#92400e' }}>Draft</span>}
                        </div>
                        <div style={{ fontSize: 12, color: '#555', marginTop: 4, fontStyle: 'italic', lineHeight: 1.4 }}>
                          {(r.scope_text || '').slice(0, 120)}{r.scope_text?.length > 120 ? '…' : ''}
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        {r.status === 'draft' && r.vendor_email && (
                          <button
                            ...
                            style={{ background: BLUE, color: 'white', border: 'none', borderRadius: 5, padding: '4px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}
                          >
                        ...
                        <button
                          ...
                          style={{ background: 'none', color: '#aaa', border: '1px solid #ddd', borderRadius: 5, padding: '4px 8px', fontSize: 11, cursor: 'pointer' }}
                        >
                          ✕
                        </button>
                      </div>
